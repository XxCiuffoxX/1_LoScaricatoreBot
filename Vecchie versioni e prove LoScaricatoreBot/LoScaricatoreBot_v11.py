...
def download_error(call):
    ...
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=translations[user_sessions]["download_error"])
...
def send_welcome(message):
    ...
    if user_id in user_sessions:
        ...
        # user_sessions[user_id] = {} #creo un array vuoto per l'utente
        ...
        user_sessions[user_id] = {'language': '', 'nome':'Mario'}
        save_sessions(user_sessions)
        language(message, user_sessions[user_id])
...
def language(message, user_session):
    ...
    bot.send_message(user_session, text="Hi, I'm LoScaricatoreBot.😉\nSend me a YT link for the magic.🪄\nBut first select a language:\n\nCiao, sono LoScaricatoreBot.😉\nInviami un link di YT per la magia.🪄\nMa prima scegli una lingua:", reply_markup = keyboard)


def set_language(user_session, language_code):

    # Imposta il valore dell'attributo "lingua"
    user_session['language'] = str(language_code)  # Sostituisci 'italiano' con il valore desiderato
    ...
    if user_id in user_sessions:
        ...
        if re.search(pattern, message.text):
            ...
            if length_control(video_url):
                ...
                bot.reply_to(message, text = f"{translations[user_session.get('language')]['video_too_long']}")
                return
            ...
            bot.reply_to(message, text=translations[user_sessions]["not_a_link"])
            return
...
def callback_query(call):
    ...
    # if the mp4 button was pressed
        ...
        set_language(session_data, language_code="it")

    elif call.data == 'en':
        set_language(session_data, language_code="en")
